...
def p(name, regime="m200", rs=85, rsacc=0, breakvol=1.2, stop=0.07):
    return v3.P(name=name, regime=regime, rs=rs, rsacc=rsacc,
                breakvol=breakvol, stop=stop, full_stack=True, vcp_shape="soft")
...
def run_one(df, ctx, param, slip=0.002):
    old = base.SLIP
    base.SLIP = slip
...
def temporal_splits(df, param):
    ...
    for name, a, b in periods:
        ...
        ctx=fast.build_context(sub)
        c,t,m=run_one(sub,ctx,param)
...
def market_benchmarks(df):
    ...
    for market in ["KOSPI","KOSDAQ"]:
        s=(df[df.Market==market][["Date","midx"]].drop_duplicates("Date").dropna().sort_values("Date").set_index("Date").midx)
        if len(s)<2: continue
        eq=s/s.iloc[0]
...
def block_bootstrap(curve, n=1000, block=20, seed=42):
    ...
    rows=[]; N=len(r); starts=np.arange(max(N-block+1,1))
    for _ in range(n):
        samp=[]
        while len(samp)<N:
            st=int(rng.choice(starts)); samp.extend(r[st:st+block])
        rr=np.array(samp[:N]); eq=np.cumprod(1+rr); yrs=N/252
        cagr=eq[-1]**(1/yrs)-1
        dd=eq/np.maximum.accumulate(eq)-1
        rows.append((cagr,dd.min()))
    z=pd.DataFrame(rows,columns=["CAGR","MDD"])
    q=z.quantile([0.05,0.25,0.50,0.75,0.95]); q.index.name="quantile"
    return q,z


def concentration(trades):
    if trades.empty: return {}
    pos=trades.loc[trades.ret>0,"ret"].sort_values(ascending=False); gross=pos.sum()
    return {
        "trades":int(len(trades)), "positive_trades":int(len(pos)),
...
def main():
    print("Preparing optimized V5 validation panel...",flush=True)
    df=v3.prepare(base.load_prepare()); ctx=fast.build_context(df)
    mainp=p("m200_acc0"); fastp=p("fast_acc0",regime="fast")

    c,t,m=run_one(df,ctx,mainp); cf,tf,mf=run_one(df,ctx,fastp)
    ...
    for strat,param in [("m200",mainp),("fast",fastp)]:
        for slip in [0.002,0.004,0.006,0.010]:
            cc,tt,mm=run_one(df,ctx,param,slip=slip)
      ...
      for rs in [80,85,90]:
       for bv in [1.0,1.2,1.4]:
        for stop in [0.06,0.07,0.08]:
            ...
            cc,tt,mm=run_one(df,ctx,pp)
            grid.append({"regime":regime,"rs":rs,"breakvol":bv,"stop":stop,**mm})
            print("grid",regime,rs,bv,stop,round(mm.get("CAGR",0),4),round(mm.get("MDD",0),4),flush=True)
    ...
    (RESULTS/"winner_concentration.json").write_text(json.dumps(conc,indent=2,ensure_ascii=False),encoding="utf-8")
    q,z=block_bootstrap(c,n=1000,block=20,seed=42)
    q.to_csv(RESULTS/"bootstrap_quantiles.csv"); z.to_csv(RESULTS/"bootstrap_samples.csv",index=False)
    ...
    print("BASE",m,flush=True); print("FAST",mf,flush=True)
    print("STABILITY",stable,flush=True); print("CONCENTRATION",conc,flush=True)
